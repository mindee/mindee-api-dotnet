...
namespace Mindee.UnitTests.V1.Product.InternationalId
{
    ...
    public class InternationalIdV2Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText(Constants.V1ProductPath + "international_id/response_v2/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<InternationalIdV2>> GetPrediction(string name)
        {
            var fileName = Constants.V1ResourcePath + $"products/international_id/response_v2/{name}.json";
