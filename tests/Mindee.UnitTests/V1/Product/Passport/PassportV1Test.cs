...
namespace Mindee.UnitTests.V1.Product.Passport
{
    ...
    public class PassportV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText(Constants.V1ProductPath + "passport/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<PassportV1>> GetPrediction(string name)
        {
            var fileName = Constants.V1ResourcePath + $"products/passport/response_v1/{name}.json";
