...
namespace Mindee.UnitTests.V1.Product.Us.HealthcareCard
{
    ...
    public class HealthcareCardV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected =
                File.ReadAllText(Constants.V1ProductDir + "us_healthcare_cards/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<HealthcareCardV1>> GetPrediction(string name)
        {
            var fileName = Constants.V1RootDir + $"products/us_healthcare_cards/response_v1/{name}.json";
