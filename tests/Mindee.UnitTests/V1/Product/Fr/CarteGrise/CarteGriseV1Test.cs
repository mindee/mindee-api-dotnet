...
using Mindee.V1.Product.Fr.CarteGrise;
...
namespace Mindee.UnitTests.V1.Product.Fr.CarteGrise
{
    ...
    public class CarteGriseV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText(Constants.V1ProductPath + "carte_grise/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<CarteGriseV1>> GetPrediction(string name)
        {
            var fileName = Constants.V1ResourcePath + $"products/carte_grise/response_v1/{name}.json";
