...
namespace Mindee.UnitTests.V1.Product.Fr.BankAccountDetails
{
    ...
    public class BankAccountDetailsV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected =
                File.ReadAllText(Constants.V1ProductPath + "bank_account_details/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<BankAccountDetailsV1>> GetPrediction(string name)
        {
            var fileName = Constants.V1ResourcePath + $"products/bank_account_details/response_v1/{name}.json";
