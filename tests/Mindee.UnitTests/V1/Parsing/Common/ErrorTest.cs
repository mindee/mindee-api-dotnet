    ...
    public class ErrorTest
    {
        ...
        public async Task Given_Details_As_object_MustBeDeserialized()
        {
            var error = await JsonSerializer.DeserializeAsync<Error>(
                new FileInfo(Constants.V1ResourcePath + "errors/with_object_response_in_detail.json").OpenRead());
        ...
        {
            var error = await JsonSerializer.DeserializeAsync<Error>(
                new FileInfo(Constants.V1ResourcePath + "errors/with_string_response_in_detail.json").OpenRead());
