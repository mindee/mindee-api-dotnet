...
namespace Mindee.UnitTests.Product.NutritionFactsLabel
{
    ...
    public class NutritionFactsLabelV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/nutrition_facts/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<NutritionFactsLabelV1>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/nutrition_facts/response_v1/{name}.json";
