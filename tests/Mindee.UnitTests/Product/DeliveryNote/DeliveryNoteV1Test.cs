...
namespace Mindee.UnitTests.Product.DeliveryNote
{
    ...
    public class DeliveryNoteV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/delivery_notes/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<DeliveryNoteV1>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/delivery_notes/response_v1/{name}.json";
