...
namespace Mindee.UnitTests.Product.InternationalId
{
    ...
    public class InternationalIdV2Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/international_id/response_v2/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<InternationalIdV2>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/international_id/response_v2/{name}.json";
