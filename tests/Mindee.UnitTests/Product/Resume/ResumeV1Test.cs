...
namespace Mindee.UnitTests.Product.Resume
{
    ...
    public class ResumeV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/resume/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<ResumeV1>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/resume/response_v1/{name}.json";
