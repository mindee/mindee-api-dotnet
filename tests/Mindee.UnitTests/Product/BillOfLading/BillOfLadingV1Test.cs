...
namespace Mindee.UnitTests.Product.BillOfLading
{
    ...
    public class BillOfLadingV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/bill_of_lading/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<BillOfLadingV1>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/bill_of_lading/response_v1/{name}.json";
