...
namespace Mindee.UnitTests.Product.Fr.EnergyBill
{
    ...
    public class EnergyBillV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/energy_bill_fra/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<EnergyBillV1>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/energy_bill_fra/response_v1/{name}.json";
