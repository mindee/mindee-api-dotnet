...
namespace Mindee.UnitTests.Product.Fr.Payslip
{
    ...
    public class PayslipV3Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/payslip_fra/response_v3/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<PayslipV3>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/payslip_fra/response_v3/{name}.json";
