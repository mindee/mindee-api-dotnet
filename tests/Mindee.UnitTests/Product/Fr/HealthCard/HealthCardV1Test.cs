...
namespace Mindee.UnitTests.Product.Fr.HealthCard
{
    ...
    public class HealthCardV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/french_healthcard/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<HealthCardV1>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/french_healthcard/response_v1/{name}.json";
