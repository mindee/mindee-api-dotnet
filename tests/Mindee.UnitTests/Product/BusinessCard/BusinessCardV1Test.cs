...
namespace Mindee.UnitTests.Product.BusinessCard
{
    ...
    public class BusinessCardV1Test
    {
        ...
        public async Task Predict_CheckSummary()
        {
            ...
            var expected = File.ReadAllText("Resources/products/business_card/response_v1/summary_full.rst");
            Assert.Equal(expected, response.Document.ToString());
        }

        private static async Task<PredictResponse<BusinessCardV1>> GetPrediction(string name)
        {
            string fileName = $"Resources/products/business_card/response_v1/{name}.json";
